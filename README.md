...
> 当前 macOS 版本：`1.9.10`（独立 `macos` 版本线）
>
...
- 支持在 macOS Keychain 中安全保存多个 Codex 账号，可从主卡片或托盘快捷切换。
- 账号窗口分别展示每个已保存账号的每周剩余额度，方便先查看再切换。
- macOS 主卡片与账号窗口使用透明背景和 WebKit 模糊，颜色会跟随当前账号的真实 5 小时额度变化。
- 不包含遥测、分析或第三方追踪。
