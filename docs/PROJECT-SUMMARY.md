...
Quota Beacon 是一个独立维护的 Tauri 2 桌面悬浮窗，用本机 Codex Desktop 登录态只读查询 Codex 额度，并以小卡片展示 5 小时额度、每周额度、重置时间、本机 Token 汇总和可定制的颜色主题。
...
- 跨平台代码：Windows 与 macOS 共享前端 UI/动效和业务逻辑，但使用独立分支、版本标签与发布包。
...
- `.github/workflows/release.yml`：在 `macos-v*` 标签上仅生成带 DMG、SHA-256 的 macOS Universal ad-hoc 发布包。
