      ...
      <section className="account-switcher__body" aria-live="polite">
        ...
        {vault?.profiles.map((profile) => (
          <article className={`account-row${profile.isActive ? " account-row--active" : ""}`} key={profile.id}>
            ...
            <button type="button" className="account-icon-button" disabled={busyId !== null} aria-label={`${t.rename} ${profile.alias}`} title={t.rename} onClick={() => { const next = window.prompt(t.alias, profile.alias); if (next !== null) void run(profile.id, () => renameAccount(profile.id, next)); }}><PencilSimple /></button>
            <button type="button" className="account-icon-button account-icon-button--danger" disabled={busyId !== null} aria-label={`${t.remove} ${profile.alias}`} title={t.remove} onClick={() => { if (window.confirm(`${t.remove} ${profile.alias}?`)) void run(profile.id, () => deleteAccount(profile.id)); }}><Trash /></button>
          </article>
        ))}
      </section>
